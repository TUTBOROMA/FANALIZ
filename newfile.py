...
u = ''
yad = None
tmr = False      # Таймер активен
tst = 0          # время старта таймера
tdur = 0         # длительность таймера в мс
calc_scroll = 0
ar_scroll = 0
m_y = 0
m_m = 0
...
le_data = {}              # данные для ввода крупной траты
le_result_global = {}     # результат расчёта крупной траты
history_days_global = 10  # количество дней для расчёта истории (по умолчанию)
...
# Функция для получения дневного накопительного итога по датам
def daily_diff(dt):
...
# Рассчитываем прогноз на основе истории за последние N дней
...
# Для отображения календаря месяца (используется в анализе месяца)
...
# Рисуем линейный график с подписями осей
...
# Рисуем столбчатую диаграмму с подписями осей
...
# Функции для интерфейса ввода данных
...
# Для расчёта истории – ввод количества дней истории
def hbi():
    global ca, ibs, idat
...
# Новая кнопка "Ссылки" – открывает справочную страницу с материалами
def links():
    global ca
    ca = 'links'

# Применяем вводимые данные
def apply_in(i, txt):
...
def main():
    ...
    # Основные кнопки главного меню (два столбца)
    mbs = [
    ...
    # Дополнительная кнопка "Ссылки" – расположена в третьем столбце справа, зелёного цвета
    extra_btns = [Btn("Ссылки", 750, 120, 300, 50, links)]
    extra_btns[0].bg = (0,200,0)
    extra_btns[0].hover_bg = (0,255,0)

    # Кнопка "Отмена" для ввода
    cb = Btn("Отмена", 500, 300, 150, 50, ci)
    # Универсальная кнопка "Назад" – будет позиционироваться в правом нижнем углу
    ...
    while run:
        dt_ = clk.tick(30)
        # Обновляем позицию кнопки "Назад" в правом нижнем углу
        ...
        for ev in pygame.event.get():
            ...
            if ev.type == QUIT:
                ...
                run = False
                break
            # Обработка событий для экранов с кнопкой "Назад"
            if ca in ['show_plans','all_reports','calc_future','more_info',
                    ...
                    if res is not None:
                        ...
                        if i_index+1 < len(ibs):
                            ...
                            if ca == 'start_timer':
                                ...
                                # Не вызываем fin(), т.к. apply_in уже сменил ca на 'history_based_forecast_result'
                                ibs = []; idat = {}
                            else:
                                fin()
                                ca = 'menu'
                ...
                if ev.type == MOUSEBUTTONDOWN and ev.button == 1:
                    if bb.hov(pygame.mouse.get_pos()):
                        ...
                        ca = 'future_analysis_pro'
        sc.fill((245,245,245))
        # Отрисовка информации о празднике по центру внизу
